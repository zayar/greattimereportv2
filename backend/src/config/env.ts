...
const envSchema = z.object({
  ...
  AI_AGENT_MONITORING_ADMIN_EMAILS: z.string().default("zayar@datafocus.cloud"),
  ...
  MASK_PHONE_IN_GROUP_CHAT: booleanEnv(true),
  APPOINTMENT_BUTTON_PHONE_SUFFIX_DIGITS: z.coerce.number().int().min(2).max(6).default(3),
  MAX_APPOINTMENT_BUTTONS_PER_PAGE: z.coerce.number().int().min(1).max(20).default(8),
  ...
  AGENT_SEMANTIC_PLANNER_ENABLED: booleanEnv(true),
  AGENT_SEMANTIC_PLANNER_MODEL: z.string().default("gemini-3.5-flash"),
  ...
  AGENT_COMPLETED_DAY_SNAPSHOT_ENABLED: booleanEnv(true),
  AGENT_OWNER_DAILY_BRIEF_ENABLED: booleanEnv(true),
